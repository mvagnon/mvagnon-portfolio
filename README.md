...
Project entries include a required creation datetime, a required cover image for the `/` hover preview, a required image gallery, and optional `github` and `url` links. Projects are sorted by creation datetime in descending order, and background colors are generated from that sorted position.
